...
query_run_main_method () {
 ...
 QUERY_SCRIPT="$QUERY_DIR/$QUERY_NAME.sql"
  ...
  #step 2.  spark TrainLogistic  :  Train logistic regression model
  #step 3.  spark calc log_reg   :  Calculating logistic regression for input data
  #step 4.  spark dump > hdfs/res:  Converting result and copy result do hdfs query result folder
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 2 ]] ; then
    ...
    ################################
    echo "========================="
    echo "$QUERY_NAME Step 2/3: logistic regression with spark-mllib (DataFrame) with direct metastore access"
    echo "========================="
    ...
    if [[ -z "$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK" || "$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK" == "spark" ]]; then
      job_jar="${BIG_BENCH_HDFS_ABSOLUTE_PATH}/${BIG_BENCH_HDFS_QUERY_RESOURCES}/bigbench-ml-spark-2x.jar"
    ...
    elif [[ -z "$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK" || "$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK" == "spark-2.4" ]]; then
          job_jar="${BIG_BENCH_HDFS_ABSOLUTE_PATH}/${BIG_BENCH_HDFS_QUERY_RESOURCES}/bigbench-ml-spark-2.4.jar"
    else
      echo "BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK parameter has no matching implementation or was empty: $BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK  "
      return 1
    fi

    main_class="io.bigdatabenchmark.v2.hwc.queries.q05.LogisticRegression"
    cmd="$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK_SPARK_BINARY --jars ${HIVE_WH_CONNECTOR_PATH} --class ${main_class} ${job_jar} ${input} -o "${output}" $parameters --saveClassificationResult false --saveMetaInfo true --verbose false"
